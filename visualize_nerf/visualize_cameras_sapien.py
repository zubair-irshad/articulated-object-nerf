...
def visualize_cameras(train_cam, test_cam, img_files_train, img_files_val):
    ...
    H=240
    focal = 0.5*H/np.tan(0.5*train_cam['camera_angle_x'])
    fig = pv.figure()
    all_c2w_train = []
    frustums = []
    img_size = (320, 240)
    for img_file in img_files_train:
        C2W = np.array(train_cam['frames'][img_file.split('.')[0]])
        #frustums.append(get_camera_frustum(img_size, focal, C2W, frustum_length=0.3, color=(0,0,1)))
        all_c2w_train.append(C2W)
    all_c2w_train = np.array(all_c2w_train)  

    all_c2w_val = []
    for img_file in img_files_val:
        C2W = np.array(test_cam['frames'][img_file.split('.')[0]])
        #frustums.append(get_camera_frustum(img_size, focal, C2W, frustum_length=0.4, color=(1,0,0)))
        all_c2w_val.append(C2W)
    all_c2w_val = np.array(all_c2w_val)


    pose_scale_factor_train = 1. / np.max(np.abs(all_c2w_train[:, :3, 3]))
    pose_scale_factor_val = 1. / np.max(np.abs(all_c2w_val[:, :3, 3]))
    print("pose_scale_factor", pose_scale_factor_train, pose_scale_factor_val)
    all_c2w_train[:, :3, 3] *= pose_scale_factor_train  
    all_c2w_val[:, :3, 3] *= pose_scale_factor_val

    for C2W in all_c2w_train:
        frustums.append(get_camera_frustum(img_size, focal, C2W, frustum_length=0.08, color=(1,0,0)))

    for C2W in all_c2w_val:
        frustums.append(get_camera_frustum(img_size, focal, C2W, frustum_length=0.08, color=(1,0,0)))
    ...
    for C2W in all_c2w_train:
        fig.plot_transform(A2B=C2W, s=0.03, strict_check=False)
    ...
    #     frustums = []
    #     H=240
    #     focal = 0.5*H/np.tan(0.5*poses_dict['camera_angle_x'])
    #     all_c2w = []
    ...
    #     for C2W in all_c2w:
    #         fig.plot_transform(A2B=convert_pose(C2W), s=0.08, strict_check=False)
    ...
    directions = get_ray_directions(240, 320, focal) # (h, w, 3)
    print("directions", directions.shape)
    c2w = all_c2w_train[0]
...
if __name__ == '__main__':
    ...
    base_dir = '/home/zubairirshad/SAPIEN/sapien_single_scene'
    instance_dir = 'laptop'
    instance_id = '10211'
    degree_id = '80_degree'
    ...
    base_dir_val = os.path.join(base_dir, instance_dir, instance_id, degree_id, 'test')
    img_files_val = os.listdir(os.path.join(base_dir_val, 'rgb'))
    img_files_val.sort()
    pose_path_val = os.path.join(base_dir_val, 'transforms.json')

    sphere_radius = 1.
    
    train_cam = json.load(open(pose_path_train))
    test_cam = json.load(open(pose_path_val))
    ...
    # camera_size = 0.1
    # colored_camera_dicts = {'train': ([0, 1, 0], train_cam_dict), 'test': ([1, 0, 0], test_cam_dict)}
    # # geometry_file = os.path.join(base_dir, 'mesh_norm.ply')
    # # geometry_type = 'mesh'

    visualize_cameras(train_cam, test_cam, img_files_train, img_files_val)