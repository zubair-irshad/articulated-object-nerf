...
def visualize_cameras(colored_camera_dicts, sphere_radius, camera_size=0.1, geometry_file=None, geometry_type='mesh'):
    ...
    for type, camera_dict in pose_dict.items():
        ...
        for C2W in all_c2w:
            ...
            # all_c2w.append(C2W)
            img_size = (640, 480)
            frustums.append(get_camera_frustum(img_size, focal, convert_pose(C2W), frustum_length=0.5, color=color))
        for C2W in all_c2w:
            fig.plot_transform(A2B=convert_pose(C2W), s=0.3, strict_check=False)
