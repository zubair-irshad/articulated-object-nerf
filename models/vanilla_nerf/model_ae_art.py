...
class NeRFMLP(nn.Module):
    def __init__(
        ...
        if deformation_mlp:
            if self.enc_after:
                pos_size_deformation = input_ch + shape_latent_dim + articulation_latent_dim
            else:
                pos_size_deformation = ((max_deg_point - min_deg_point) * 2 + 1) * input_ch + shape_latent_dim + articulation_latent_dim

            init_layer_deformation = nn.Linear(pos_size_deformation, netwidth_deformation)
            ...
            for idx in range(netdepth_deformation - 1):
                module_deformation = nn.Linear(netwidth_deformation, netwidth_deformation)
            ...
            pos_size = ((max_deg_point - min_deg_point) * 2 + 1) * input_ch + shape_latent_dim
        else:
            pos_size = ((max_deg_point - min_deg_point) * 2 + 1) * input_ch + shape_latent_dim + articulation_latent_dim
        ...
        views_linear = [nn.Linear(netwidth + view_pos_size + appearance_latent_dim, netwidth_condition)]
    ...
    def forward(self, pos, condition, latents):
        ...
        embedding_instance_shape = repeat(embedding_instance_shape, "n1 c -> (n1 n2) c", n2=BN)
        embedding_instance_appearance = repeat(embedding_instance_appearance, "n1 c -> (n1 n2) c", n2=BN)
        embedding_instance_articulation = repeat(embedding_instance_articulation, "n1 c -> (n1 n2) c", n2=BN)

        x = torch.cat([pos, embedding_instance_shape, embedding_instance_articulation], -1)
        ...
        x = torch.cat([bottleneck, condition_tile, embedding_instance_appearance], dim=-1)
...
class LitNeRF_AE_ART(LitModel):
    def __init__(
        ...
        super(LitNeRF_AE_ART, self).__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred_state = self.model.joint_state_decoder(embedding_instance_articulation.squeeze(0))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        state_error = self.mse_error(pred_state.float(), batch["deg"])
        abs_state_error_deg = torch.round(torch.rad2deg(pred_state)) - torch.round(torch.rad2deg(batch["deg"]))

        self.log("val/state_error_rad", state_error.item(), on_epoch=True, sync_dist=True)
        self.log("val/abs_state_error_deg", abs_state_error_deg, on_epoch=True, sync_dist=True)
    ...
    def opacity_loss_autorf(self, rendered_results, instance_mask):
        ...
        if bg_mask.sum() > 0:
            bg_loss_course = (pred_op_course[bg_mask].mean() * bg_ratio)  # l2 error to bg -> depth =0
            bg_loss_fine = (pred_op_fine[bg_mask].mean() * bg_ratio)  # l2 error to bg -> depth =0
            loss+=bg_loss_course
            loss+=bg_loss_fine
        ...
        if fg_mask.sum() > 0:
            fg_loss_course = (fg_ratio *(1 - pred_op_course[fg_mask]).mean())  # l2 error on sum of fg weights => 1
            fg_loss_fine = (fg_ratio *(1 - pred_op_course[fg_mask]).mean())  # l2 error on sum of fg weights => 1
            loss+=fg_loss_course
            loss+=fg_loss_fine
        return loss
