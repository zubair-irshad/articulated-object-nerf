...
def gen_articulated_object_nerf_s1(num_pos_img, radius_, split, camera, asset, scene, object_path, camera_mount_actor=None, theta_range = [0*math.pi, 2*math.pi], phi_range = [0*math.pi, 1*math.pi]):
    ...
    for i in tqdm(range(num_pos_img)):
        ...
        # point = points[i]
        ret_dict = render_pose(point, instance_save_path, camera_mount_actor, scene, camera, asset, pose_fn=custom_openGL, save=False)
        frame_id = 'r_'+str(i)
